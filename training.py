...
def train_FAD(args):
    ...
    optimizer = torch.optim.Adam(params = chain(FAD_2Dto3D.parameters(), FAD_3Dto2D.parameters()))
    ...
    for epoch in trange(args.epochs_no, desc = f'Training Feature Transfer Net.'):
        ...
        for (rgb, pc, _), _ in tqdm(train_loader, desc = f'Extracting feature from class: {args.class_name}.'):
            ...
            # Make FADs trainable.
            FAD_2Dto3D.train(), FAD_3Dto2D.train()
    ...
    torch.save(FAD_2Dto3D.state_dict(), os.path.join(directory, 'FAD_2Dto3D_' + model_name + '.pth'))
    torch.save(FAD_3Dto2D.state_dict(), os.path.join(directory, 'FAD_3Dto2D_' + model_name + '.pth'))
