...
def infer_FAD(args):
    ...
    # Model instantiation. 
    FAD_2Dto3D = FeatureProjectionMLP(in_features = 768, out_features = 1152)
    FAD_3Dto2D = FeatureProjectionMLP(in_features = 1152, out_features = 768)

    FAD_2Dto3D_path = rf'{args.checkpoint_folder}/{args.class_name}/FAD_2Dto3D_{args.class_name}_{args.epochs_no}ep_{args.batch_size}bs.pth'
    FAD_3Dto2D_path = rf'{args.checkpoint_folder}/{args.class_name}/FAD_3Dto2D_{args.class_name}_{args.epochs_no}ep_{args.batch_size}bs.pth'

    FAD_2Dto3D.load_state_dict(torch.load(FAD_2Dto3D_path))
    FAD_3Dto2D.load_state_dict(torch.load(FAD_3Dto2D_path))

    FAD_2Dto3D.to(device), FAD_3Dto2D.to(device)

    # Make FADs non-trainable.
    FAD_2Dto3D.eval(), FAD_3Dto2D.eval()
